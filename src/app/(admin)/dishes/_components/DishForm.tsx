...
export default function DishForm({ categories, initialData, action = createDish }: Props) {
    ...
    return (
        <form action={formAction} className="bg-white p-6 rounded-xl shadow-sm dark:bg-gray-800 mx-auto flex flex-col gap-6">
            ...
            <div className="flex items-center justify-end gap-3 mt-8 pt-4 border-t border-gray-100 dark:border-gray-700 select-none">
                <Link
                    href="/dishes"
                    className="inline-flex items-center justify-center px-5 py-3.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-500 dark:bg-gray-800 dark:text-gray-200 dark:border-gray-600 dark:hover:bg-gray-700 transition"
                >
                    Cancelar
                </Link>
                <Button disabled={isPending || isUploadingImages} className="min-w-[160px]">
